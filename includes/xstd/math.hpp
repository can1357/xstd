...
namespace xstd::math
{
	...
	inline constexpr mat4x4 inverse( const mat4x4& m, float& det )
	{
		...
		if ( !std::is_constant_evaluated() && !is_consteval( m ) )
		{
			...
			__m256 tmp01, tmp23;
			impl::load_matrix( m.m, tmp01, tmp23 );
			__m256 tmp01t = tmp01;
			__m256 tmp23t = tmp23;
			impl::tranpose_inplace( tmp01, tmp23 );
			__m128 col0 = _mm256_extractf128_ps( tmp01, 0 );
			__m128 col1 = _mm256_extractf128_ps( tmp01, 1 );
			__m128 col2 = _mm256_extractf128_ps( tmp23, 0 );
			__m128 col3 = _mm256_extractf128_ps( tmp23, 1 );
			__m128 row0 = _mm256_extractf128_ps( tmp01t, 0 );
			__m128 row1 = _mm256_extractf128_ps( tmp01t, 1 );
			__m128 row2 = _mm256_extractf128_ps( tmp23t, 0 );
			__m128 row3 = _mm256_extractf128_ps( tmp23t, 1 );
			__m128 detv, tmp1;
			...
			col3 = row1 * tmp1 + col3;
			detv = row0 * col0;
			detv = __builtin_shufflevector( detv, detv, 2, 3, 4, 5 ) + detv;
			detv = __builtin_shufflevector( detv, detv, 1, 0, 7, 6 ) + detv;
			det = detv[ 0 ];
			detv = 1.0f / detv;
			col0 = col0 * detv;
			col1 = col1 * detv;
			...
			return {
				vec4::from_xvec( xvec<float, 4>{ std::in_place, col0 } ),
				vec4::from_xvec( xvec<float, 4>{ std::in_place, col1 } ),
				vec4::from_xvec( xvec<float, 4>{ std::in_place, col2 } ),
				vec4::from_xvec( xvec<float, 4>{ std::in_place, col3 } )
			};
		}
#endif
